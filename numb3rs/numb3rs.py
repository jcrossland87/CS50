# Week 7 Regular Expressions Problem Set 7 numb3rs.py with test_numb3rs.py
#
...
# typically formatted in dot-decimal notation as #.#.#.#. But each # should be a number between 0 and 255, inclusive.
...
# two or more functions that collectively test your implementation of validate thoroughly, each of whose names should begin with
...
# else pytest will warn with DeprecationWarning: invalid escape sequence. Just as format strings are prefixed with f, so are raw strings prefixed with r.
# For instance, instead of "harvard\.edu", use r"harvard\.edu".
# Note that re.search, if passed a pattern with “capturing groups” (i.e., parentheses), returns a “match object,” per docs.python.org/3/library/re.html#match-objects,
# wherein matches are 1-indexed, which you can access individually with group, per docs.python.org/3/library/re.html#re.Match.group, or collectively with groups,
# per docs.python.org/3/library/re.html#re.Match.groups.
#
# How to Test
    ...
    if re.search("^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$", ip):
        ...
        for number in list_of_numbers:
            if int(number) < 0 or int(number) > 255:
                return False
        return True
# uncommented out return False to make a passing pytest test_numb3rs.py
    else:
        return False
